...
def create_supervised_trainer(cfg, model, optimizer, criterion, scheduler, cetner_loss_weight=0.0, device=None):
    """
    ...
    def _update(engine, batch):
        ...
        score, feat, y = model(img)
        loss = criterion['total'](score, feat, target)
        local_loss = 0
        for score in y:
            loss_one = criterion['xent'](score, target)
            local_loss = local_loss + loss_one
        loss = loss + 0.4 * local_loss
        loss.backward()
        ...
        local_acc = []
        for score in y:
            local_acc_one = (score.max(1)[1] == target).float().mean()
            local_acc.append(local_acc_one)
        acc = (acc + (sum(local_acc)/len(local_acc)))/2
...
def do_train(
    ...
    logger = logging.getLogger("MAFFN")
    logger.info("Start training")

    trainer = create_supervised_trainer(cfg, model, optimizer, criterion, scheduler, cfg.SOLVER.CENTER_LOSS_WEIGHT,
                                        device=device)
    ...
    if cfg.TEST.PARTIAL_REID == 'off':
        ...
        evaluator_reid = create_supervised_evaluator(model,
                                                     metrics={'r1_mAP_mINP': r1_mAP_mINP(300, max_rank=50,
                                                                                         feat_norm=cfg.TEST.FEAT_NORM)},
                                                     device=device)
        evaluator_ilids = create_supervised_evaluator(model,
                                                      metrics={'r1_mAP_mINP': r1_mAP_mINP(119, max_rank=50,
                                                                                          feat_norm=cfg.TEST.FEAT_NORM)},
                                                      device=device)
    checkpointer = ModelCheckpoint(output_dir, cfg.MODEL.NAME, save_interval=checkpoint_period, n_saved=10,
                                   require_empty=False)
    ...
    def log_training_loss(engine):
        ...
        if ITER % log_period == 0:
            logger.info("Epoch[{}] Iteration[{}/{}] loss: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(engine.state.epoch, ITER, len(data_loader['train']), engine.state.metrics['avg_loss'],
